...
words = ['дурак', 'тупой', 'козел', 'овца', 'ублюдок', 'идиот', 'засранец', 'простофиля', 'черт']
...
async def user_warning(message: types.Message):
    if message.chat.type != 'private':
        if message.from_user.id not in admin:
            ...
            user_id = message.reply_to_message.from_user.id.id
            user_name = message.reply_to_message.from_user.full_name
            user_warnings[user_id] = user_warnings.get(user_id, 0) + 1

            for Admin in admin:
                await bot.send_message(chat_id=Admin,
                                       text=f'{user_name} пользователь получил предупреждение ({user_warnings[user_id]}/3)')
                ...
                if user_warnings[user_id] < 3:
                    ...
                    await bot.send_message(chat_id=message.chat.id,
                                           text=f'{user_name} был удален за превышение количество предупреждение!')
